...
class BaseDiskFileManager(object):
    ...
    def get_diskfile(self, device, partition, account, container, obj,
        ...
        return self.diskfile_cls(self, dev_path,  # 1606
...
class BaseDiskFileReader(object):
    ...
    def __init__(self, fp, data_file, obj_size, etag,
                 ...
                 quarantine_hook, use_splice, pipe_size, diskfile, offset=0, read_size=-1, offset_queue=[],
                 keep_cache=False):
        # Parameter tracking
        self._fp = fp
    ...
    def set_pre_offset_to_read(self, offset, read_size):
        self.offset = offset
        self.read_size = read_size
    ...
    def __iter__(self):
        ...
        try:
            ...
            self._read_to_eof = False
            self._init_checks()
            if 0 < len(self.offset_queue): # means that needs to cache this merged file
                print 'offset : ' + str(len(self.offset_queue))
                for iterator in range(len(self.offset_queue) - 1):
                    if self.offset_queue[iterator] == '':
                        continue
                    # print 'read size : ' + str(self.read_size)
                    chunk = self._fp.read(int(self.offset_queue[iterator + 1]) - int(self.offset_queue[iterator]))
                    # print chunk
                    # print 'self._disk_chunk_size' + str(self._disk_chunk_size) + 'self.read_size' + str(self.read_size)
                    if chunk:
                        self._update_checks(chunk)
                        self._bytes_read += len(chunk)
                        # print 'self.read_size' + str(self.read_size) + 'self._bytes_read' + str(self._bytes_read)
                        ...
                        self._drop_cache(self._fp.fileno(), dropped_cache,
                                         self._bytes_read - dropped_cache)
                        # print 'I should be broken here'
                        break

            if 0 == len(self.offset_queue):  # means that no needs to cache
                self._fp.read(self.offset)
                while True:
                    # print 'read size : ' + str(self.read_size)
                    chunk = self._fp.read(min(self._disk_chunk_size, self.read_size))
                    # print 'self._disk_chunk_size' + str(self._disk_chunk_size) + 'self.read_size' + str(self.read_size)
                    if chunk and self.read_size != 0:
                        self._update_checks(chunk)
                        self._bytes_read += len(chunk)
                        ...
                        #print 'I should be broken here'
                        break
            #print 'offset : \'s ' + str(self.offset) + ' Object reads finished'
        finally:
...
class BaseDiskFile(object):
    ...
    def __init__(self, mgr, device_path, partition,
        ...
        if _datadir:
            ...
            name_hash = hash_path(account, container, obj)
            print 'account : ' + account + 'container : ' + container + 'obj : ' + obj
            self._datadir = join(
                device_path, storage_directory(get_data_dir(policy),
                                               partition, name_hash))
            print 'data_dir = ' + self._datadir
    ...
    def _construct_from_data_file(self, data_file, meta_file, ctype_file,
        ...
        if meta_file:
            ...
            print('Content-Length in metadata : ' + sys_metadata['Content-Length'])
    ...
    def reader(self, keep_cache=False, offset=0, read_size=9999999999, offset_queue=[],
        ...
        dr = self.reader_cls(
            ...
            , offset=offset, read_size=read_size,offset_queue=offset_queue)
            ...
            if size is not None and size > 0:
                try:
                    ...
                    if err.errno in (errno.ENOSPC, errno.EDQUOT):
                        raise DiskFileNoSpace()
                    raise
            print 'self._name : ' + self._name + 'self._datadir : ' + self._datadir
...
class DiskFile(BaseDiskFile):
    ...
    def set_pre_offset_to_read(self, offset, read_size):
        self.reader_cls.set_pre_offset_to_read(offset, read_size)
