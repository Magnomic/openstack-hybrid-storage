...
import swift.common.HashTable as HashTable
...
class ObjectController(BaseStorageServer):
    ...
    def PUT(self, request):
        ...
        order_in_big_file = request.headers.get('order_in_big_file')
        print(str(order_in_big_file))
        if str(order_in_big_file) != '-1':
            return HTTPSmallFile(request=request, etag='Small File No Save')
        ...
        order_name_queue = request.headers.get('order_name_queue')
        print 'order_name_queue : ' + order_name_queue
        offset_queue = request.headers.get('offset_queue')
        merge_helper = file_merge.fileMerge()
        for obj_ite in order_name_queue.split('|'):
            merge_helper.set_small_big_map(obj_ite, obj)
        print(order_name_queue)
        merge_helper.set_big_position(obj, [device, partition])
        ...
        print str(request.if_none_match)
        ...
        etag = md5()
        elapsed_time = 0
        # print request.headers['File-Name']
        # print request.headers['file_bytes_transferred']
        # ------------------------------------------------------
        ...
        try:
            with disk_file.create(size=fsize) as writer:
                ...
                hundred_continue_headers = []  # useless from here
                if config_true_value(
                        request.headers.get(
                ...
                if have_metadata_footer or use_multiphase_commit:
                    ...
                    print (hundred_continue_headers)
                    ...
                    try:
                        ...
                        return HTTPRequestTimeout(request=request)  # useless stop here
                timeout_reader = self._make_timeout_reader(obj_input)

                self.logger.info('timeout_reader finished')
                try:
                    for chunk in iter(timeout_reader, ''):
                        start_time = time.time()
                        if start_time > upload_expiration:
                        ...
                        upload_size = writer.write(chunk)
                        elapsed_time += time.time() - start_time
                        # self.logger.info('chunk use time ' + str(elapsed_time) + ' for obj ' + obj)
                except ChunkReadError:
                    return HTTPClientDisconnect(request=request)
                except ChunkReadTimeout:
                    return HTTPRequestTimeout(request=request)
                self.logger.info('chunk read finished')
                if upload_size:
                    self.logger.transfer_rate(
                        'PUT.' + device + '.timing', elapsed_time,
                        upload_size)
                ...
                etag = etag.hexdigest()
                if request_etag and request_etag != etag:  # verify md5
                ...
                metadata = {
                    ...
                    'order_in_big_file': order_in_big_file,
                    'order_name_queue': order_name_queue,
                ...
                writer.commit(request.timestamp)  # no execute
    ...
    def GET(self, request):
        ...
        cache = None
        # --------------------- get small file ----------------------------
        cache_helper = file_merge.fileMerge()
        disk_file = self.get_diskfile(
            device, partition, account, container, obj,
            policy=policy, frag_prefs=frag_prefs)
        self.logger.info('device : ' + device + 'partition : ' + partition + 'obj : ' + obj)
        disk_file_for_big_file = None
        small_file_name = None
        try:
            # --------------------- get big file ----------------------------
            small_file_name = request.headers.get('small_file_name')
            obj_for_big_file = cache_helper.get_small_big_map(small_file_name)
            device_for_big_file, partition_for_big_file = cache_helper.get_big_position(obj_for_big_file)
            disk_file_for_big_file = self.get_diskfile(
                device_for_big_file, partition_for_big_file, account, container, obj_for_big_file,
                policy=policy, frag_prefs=frag_prefs)
            # --------------------- read to cache ----------------------------
            start_time = time.time()
            cache = cache_helper.mc_get(small_file_name)
            # cache = False
            # print(cache)
            # disk_file_for_big_file.open()
            # cache_helper.put_to_cache(obj_for_big_file, disk_file_for_big_file)
            # --------------------- end --------------------
        except DiskFileDeviceUnavailable:
            return HTTPInsufficientStorage(drive=device, request=request)
        except KeyError:
            print('Not shown in training set')
        try:

            # if not cache:
            #     cache = cache_helper.send_message(disk_file_for_big_file._datadir, small_file_name)
            if cache:
            #     # pass
                metadata, durable_timestamp = cache_helper.mc_get_meta(small_file_name)
                if not metadata:
                    with disk_file.open():
                        metadata = disk_file.get_metadata()
                        if not durable_timestamp:
                            durable_timestamp = disk_file.durable_timestamp
                        cache_helper.mc_set_meta(small_file_name, metadata, disk_file.durable_timestamp)
                ...
                response.headers['X-Backend-Data-Timestamp'] = \
                    Timestamp(metadata.get('X-Timestamp')).internal
                if durable_timestamp:
                    response.headers['X-Backend-Durable-Timestamp'] = \
                        durable_timestamp.internal
                response.headers['X-Backend-Fragments'] = None
                resp = request.get_response(response)
            else:
                current_hot = hash_table.check_hot(small_file_name)
                # for some device dose not have ssd cache
                current_hot = False
                if current_hot:
                    disk_file = self.get_diskfile(
                        'sdb', partition, account, container, obj,
                        policy=policy, frag_prefs=frag_prefs)
                    metadata, durable_timestamp = cache_helper.mc_get_meta(small_file_name)
                    with disk_file.open():
                        obj_size = int(metadata['Content-Length'])
                    ...
                    with disk_file.open():
                        ...
                        cache_helper.mc_set_meta(small_file_name, metadata, disk_file.durable_timestamp)
                        resp = request.get_response(response)
                # add to cache after handling the data reading process above
                # if disk_file_for_big_file is not None:

                # add hot record
                is_hot = hash_table.add_access(small_file_name)
                # is_hot = True
                # if the file is a new hot file, move it to ssd
                if not current_hot and is_hot:
                    # add metadata to cache
                    print(metadata)
                    print(type(disk_file.durable_timestamp))
                    cache_helper.mc_set_meta(small_file_name, metadata, disk_file.durable_timestamp)
                    print('hot detected')
                    # send message of moving file to ssd to cache thread
                cache_helper.send_message(disk_file_for_big_file._datadir, disk_file._datadir, small_file_name, not current_hot and is_hot)
