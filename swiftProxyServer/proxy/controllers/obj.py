...
import swift.common.file_merge as file_merge
...
class BaseObjectController(Controller):
    ...
    def GETorHEAD(self, req):
        ...
        try:
            ...
            # for key in self.app.small_big_map:
            #     print 'map ' + key + ' : ' + self.app.small_big_map[key]
            file_item = file_merge.fileMerge()
            small_file_name = self.object_name
            # file_merge.print_map()
            big_object_name = file_item.get_small_big_map(small_file_name)  # if the object is a small file
            if big_object_name == file_item.get_big_file_name():  # if the big object is pending for upload
                return HTTPNotFound(status=404)
            req.headers['small_file_name'] = small_file_name  # tell object server which small file to get
            req.headers['big_file_name'] = big_object_name
        except KeyError:
            print('big_object key error')
            req.headers['small_file_name'] = ''  # if the object is a big fil
            req.headers['big_file_name'] = ''  # if the object is a big fil
    ...
    def _connect_put_node(self, nodes, part, req, headers,
        ...
        for node in nodes:
            try:
                ...
                # print "Stopped here" + str(node)
                self.app.exception_occurred(
                    node, _('Object'),
                    _('Monitoring Expect: 100-continue on %s') % req.swift_entity_path)
    ...
    def _store_object(self, req, data_source, nodes, partition,
        ...
        :param data_source: an iterator of the source of the data # we may need to concentrate on it
        :param nodes: an iterator of the target storage nodes # it is a list
    ...
    @cors_validation
    @delay_denial
    def PUT(self, req):  # global temp message is in server.py : about 120
        """HTTP PUT request handler."""
        # print ("Yes I got here!") #i changed here
        print 'received : ' + self.object_name
        if req.if_none_match is not None and '*' not in req.if_none_match:  # it seems that we won't use here
            # Sending an etag with if-none-match isn't currently supported
            return HTTPBadRequest(request=req, content_type='text/plain',
                                  body='If-None-Match only supports *')
        container_info = self.container_info(
            self.account_name, self.container_name, req)
        # print(req.headers)#i changed here
        policy_index = req.headers.get('X-Backend-Storage-Policy-Index',
                                       container_info['storage_policy'])
        obj_ring = self.app.get_object_ring(policy_index)
        container_nodes = container_info['nodes']
        container_partition = container_info['partition']
        is_message_upload = req.headers.get('Upload-Message')
        # object_name = time.time()

        # print(partition)
        # print(nodes)  #i changed here
        # pass the policy index
        # to storage nodes via req header
        req.headers['X-Backend-Storage-Policy-Index'] = policy_index
        req.headers['File-Name'] = self.object_name
        req.acl = container_info['write_acl']
        req.environ['swift_sync_key'] = container_info['sync_key']

        # is request authorized
        if 'swift.authorize' in req.environ:  # should not be changed
            aresp = req.environ['swift.authorize'](req)
            if aresp:
                return aresp
        ...
        if error_response:
            return error_response  # check error, no relation with real storage process

        self._update_x_timestamp(req)  # Used by container sync feature

        # print (self.app.client_chunk_size)

        def reader():
            try:
                # print req.environ['wsgi.input']
                return req.environ['wsgi.input'].read(  # class InputProxy(object):
                    self.app.client_chunk_size)
            except (ValueError, IOError) as e:
                raise ChunkReadError(str(e))

        data_source = iter(reader, '')  # read until meet ''
        # -------------------------------------------------------
        ...
        while True:
            try:
                ...
                duplicated_data_source.append(chunk)
                # file_merge.append_chunk(chunk)
                length += len(chunk)
            except StopIteration:
                break
        # size_in_chunk = file_merge.get_size_in_chunk()
        # order_in_chunk = file_merge.get_order_in_chunk()
        # queue_in_chunk = file_merge.get_queue_in_chunk()
        # queue_in_offset = file_merge.get_queue_in_offset()
        # if file_merge.get_big_file_name() is None:
        #     size_in_chunk = 0
        #     order_in_chunk = 0
        #     queue_in_chunk = ''
        #     file_merge.set_big_file_name(str(time.time()) + str(random.random()))
        # file_merge.set_small_big_map(self.object_name, file_merge.get_big_file_name())
        # file_merge.append_chunk_list(duplicated_data_source)
        # # print 'PUT_small file :' + self.object_name
        # # print 'PUT_big file :' + self.app.big_file_name
        # file_merge.set_order_in_chunk(order_in_chunk + 1)
        # file_merge.set_size_in_chunk(size_in_chunk + length)
        # file_merge.set_queue_in_chunk(queue_in_chunk + '|' + self.object_name)
        # file_merge.set_queue_in_offset(queue_in_offset + '|' + str(size_in_chunk))
        #
        #
        file_item = file_merge.fileMerge()
        is_small_file, big_file_name, big_file, order_name_queue, offset_queue \
            = file_item.handle_object(self.object_name, duplicated_data_source, length, is_message_upload)
        # if size_in_chunk + length < 1024*1024*1:
        if is_small_file:
            return Response(status=204)
        else:
            # req.headers['big_file_name'] = file_merge.get_big_file_name()
            req.headers['big_file_name'] = big_file_name
            # big_file = file_merge.get_chunk()
            # file_merge.clear_chunk()
            duplicated_data_source = big_file
            req.headers['order_in_big_file'] = -1
            # req.headers['order_name_queue'] = file_merge.get_queue_in_chunk()
            req.headers['order_name_queue'] = order_name_queue
            print(order_name_queue)
            # file_merge.set_queue_in_offset(file_merge.get_queue_in_offset() + '|' + str(size_in_chunk + length))
            # req.headers['offset_queue'] = file_merge.get_queue_in_offset()
            req.headers['offset_queue'] = offset_queue
            self.object_name = big_file_name
            # file_merge.set_order_in_chunk(0)
            # file_merge.set_size_in_chunk(0)
            # file_merge.set_queue_in_chunk('')
            # file_merge.set_queue_in_offset('')
            # file_merge.set_big_file_name(None)

        # -------------------------------------------------------
        # the partition should not calculate at first time, when receive enough requests, save them into a bigger one
        # however we can generate an virtual file to save them, we first generate a timestamp based file name
        partition, nodes = obj_ring.get_nodes(
            self.account_name, self.container_name, self.object_name)

        path_list = req.path_info.split('/', 4)[:4]
        path_list.append(self.object_name)
        req.path_info = '/'.join(path_list)
        print 'pathinfo : ' + req.path_info
        # print req.environ['wsgi.input'].read(-1)
        ...
        resp = self._store_object(
            req, duplicated_data_source, nodes, partition, outgoing_headers, 'PUT')
        return resp
...
class ReplicatedObjectController(BaseObjectController):
    ...
    def _transfer_data(self, req, data_source, putters, nodes, type=None):
        """
        ...
        # print (putters)
        #  generate a buffer data to transfer data, if the data is a merged data, transfer it to Object Server
        #  if is a empty data, give it a special header and the Object Server will handle it
            ...
            with ContextPool(len(nodes)) as pool:
                ...
                if type is None:
                    ...
                    for chunk in data_source:
                        ...
                        send_chunk(chunk)
                #  print 'req.content_length' + req.content_length
                # print 'The request transferd : ' + str(bytes_transferred)
                #  req.headers['file_bytes_transferred'] = bytes_transferred
    ...
    def _store_object(self, req, data_source, nodes, partition,
        ...
        """
        # print ("_store_object runs on here!")  # i changed here
        policy_index = req.headers.get('X-Backend-Storage-Policy-Index')
        policy = POLICIES.get_by_index(policy_index)
        if not nodes:
            return HTTPNotFound()
        ...
        min_conns = quorum_size(len(nodes))  # at least (n+1)//2 connections success
        try:
...
def client_range_to_segment_range(client_start, client_end, segment_size):
    ...
    :param client_stsart: first byte of the range requested by the client
    :param client_end: last byte of the range requested by the client
    :param segment_size: ize of an EC segment, in bytes
    ...
    # print ("client_start\t" + client_start + "client_end\t" + client_end + "segment_size\t" + segment_size + "\n")
    segment_start = (
    ...
    segment_end = (
         ...
         * segment_size))
    # print ("segment_start\t" + str(segment_start) + "segment_end\t" + str(segment_end) + "\n")
        ...
        while True:
            ...
            if not self.failed:
                if self.chunked:
                    ...
                    to_send = chunk
                try:
                    # print (self.conn)  # i changed here
